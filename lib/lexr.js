...
Lex.prototype.addChunk = function(text, chunkId){
  ...
  $.each(words, function(k, word){
    if (word) lex.tally(word, chunkId);
    if (words[k+1]) {
      if (!lex.list[word].next) lex.list[word].next = new Lex();
      lex.list[word].next.tally(words[k+1], chunkId);
    }
  });
};
...
Lex.prototype.merge = function(key1, key2){
  if (!this.list[key1] || !this.list[key2]) return false; // not sure how this happens yet
  this.list[key1].count += this.list[key2].count;
  this.list[key1].ids = this.list[key1].ids.concat(this.list[key2].ids);
  delete this.list[key2];
};
