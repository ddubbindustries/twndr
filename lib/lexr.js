var lex = {
  ...
  getWeightedRandom: function(obj, excludeArr){
    ...
    out = out.filter(function(v){ return excludeArr.indexOf(v.word) == -1; });
    
    var onlyTopWords = out.filter(function(v){ return lex.isInArr(lex.meta.top, 'word', v.word);});
  ...
  output: {
    ...
    getScore: function(arr, markup){
      var score = 0, scoredWords = [], out = '';
      $.each(arr, function(k,v){
        if (lex.isInArr(lex.meta.top, 'word', v.word) && scoredWords.indexOf(v.word) == -1) {
    ...
    line: function(opt){
      ...
      for (tries=1; tries <= maxTries; tries++) {
        wordArr = lex.output.series(minCount, startWord);
        charLength = wordArr.map(function(v){ return v.word; }).join(' ').length;
        if (
          wordArr.length >= minCount && 
          wordArr.length <= maxCount &&
          charLength <= maxChars &&
          lex.output.getScore(wordArr) >= minScore
        ) break;
      }
      out = wordArr.map(function(v){
        var markup = lex.isInArr(lex.meta.top, 'word', v.word) !== false ? 'hilight' : '';
        return opt.optimize ? '<span class="'+markup+'">'+v.word+'</span>' : v.word;
      }).join(' ');
      if (opt.stats) 
        out += ' ' + '<span class="stats">'+JSON.stringify({
          ...
          tries: tries,
          time: new Date() - time,
          score: lex.output.getScore(wordArr)
        }).replace(/"/g,'') + '</span>';
