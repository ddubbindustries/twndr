var lex = {
  ...
  init: function(){
    ...
    this.meta.getTopArr = function(){return lex.objToArr(lex.meta.top).map(function(v){return v.word+' '+v.count;})};
    this.chunks = {};
    this.list = {};
  },
  ...
  output: {
    ...
    line: function(opt){
      ...
      out = bestWordArr.map(function(v){
        ...
        return opt.stats ? '<span class="'+markup+'" title="'+count+'">'+v.word+'</span>' : v.word;
      }).join(' ');
      if (opt.stats) 
