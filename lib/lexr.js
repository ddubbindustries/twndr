...
Lex.prototype.tally = function(key, chunkId, authorId, metadata){
  ...
  if (metadata) this.list[key].meta = metadata;
...
Lex.prototype.addChunk = function(text, chunkId, authorId, storeNext){
  var lex = this,
      tokens = typeof(text) == 'string' ? lex.tokenize(text) : text;

  $.each(tokens, function(k, token){
    lex.tally(token, chunkId, authorId);
    if (storeNext && tokens[k+1]) {
...
Lex.prototype.setTop = function(){
  var topArr = this.sortArrByCount(this.objToArr(this.filtered));
  this.topArr = topArr;
  this.setTopPercents();
  return this.topArr;
};
