var lex = {
  ...
  afterChunks: function(topFilterFn){
    lex.meta.uniqueCount = Object.keys(lex.list).length;
    
    var tempList = JSON.parse(JSON.stringify(lex.list)),
        hashList = {},
        plainList = {};
    ...
    $.each(plainList, function(k,v){
      if (hashList['#'+k] && util.isRelevant(k)) {
        hashList['#'+k].count += v.count;
        hashList['#'+k].ids.push.apply(hashList['#'+k].ids, v.ids);
      } else {
        hashList[k] = v;
      }
    });
    ...
    var topList = lex.objToArr(lex.list)
          .filter(function(v){
            return ( 
              util.isRelevant(v.word) &&
              (topFilterFn ? topFilterFn(v.word) : true)
            );
          });
    lex.meta.topArr = lex.sortArr(topList).filter(function(v){return v.count > 1;});
  }, 
  ...
  tally: function(item, chunkId, list){
    var inc = 1;
    if (list[item]) {
      list[item].count += inc;
